...
inline unsigned char height(node* p_node) { return p_node ? p_node->height : 0; }
inline int balance_factor(node* p_node) { return height(p_node->p_right) - height(p_node->p_left); }

inline void fix_height(node* p_node)
{
...
inline node* rotate_right(node* p_node)
{
...
inline node* rotate_left(node* p_node)
{
...
inline node* balance(node* p_node)
{
	fix_height(p_node);
...
inline node* insert(node* p_node, int key, int data)
{
	if(p_node == NULL)
	{
...
inline int get_data(node* p_node, int key)
{
...
inline node* find_min(node* p_node) { return (p_node->p_left) ? find_min(p_node->p_left) : p_node; }

inline node* remove_min(node* p_node)
{
...
inline node* remove(node* p_node, int key)
{
